...
*   **quality**: качество сохраняемого изображения, выражается в виде числа в диапазоне от 0 до 100, где 100 является обычно полным изображением без потери качества при сжатии. *(Число)* (Обратите внимание, что информация о разрешении камеры недоступна.)

*   **destinationType**: Выбирает формат возвращаемого значения. Определен в `navigator.camera.DestinationType` *(Число)*
...
*   **sourceType**: Выберите источник изображения. Определен в `navigator.camera.PictureSourceType` *(Число)*
...
*   **encodingType**: Выбирает кодировку возвращаемого файла изображения. Определен в `navigator.camera.EncodingType` *(Число)*
...
*   **cameraDirection**: Выбирает какую камеру использовать (переднюю- или заднюю). Определено в `navigator.camera.Direction` *(Число)*
...
*   При использовании `destinationType.FILE_URI` , фотографии сохраняются во временном каталоге приложения. Вы можете удалить содержимое этого каталога используя API `navigator.fileMgr` если свободное место является проблемой.
